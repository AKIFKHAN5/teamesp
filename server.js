...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const ok = /jpeg|jpg|png|gif|webp|mp4|mov|avi|mkv|webm/.test(
...
// Content store — all editable text
app.get('/api/content', (req, res) => {
  const db = readDB();
  res.json(db.content || {});
});

// Place order (supports cart: array of items)
app.post('/api/orders', (req, res) => {
  const { items, customer_name, customer_contact, payment_method, pubg_id, notes } = req.body;
  if (!items?.length || !customer_name || !customer_contact || !payment_method || !pubg_id) {
    return res.status(400).json({ error: 'All fields are required' });
  }
  const db = readDB();
  const order = {
    id: `ESP-${Date.now()}`,
    items,                       // [{tier_id, label, price_pkr, price_usdt, qty}]
...
app.post('/api/chat', (req, res) => {
  ...
  if (msg.includes('price') || msg.includes('cost') || msg.includes('rate') || msg.includes('kitna')) {
    reply = `💰 <b>Current Prices:</b><br>${p.tiers.map(t=>`• ${t.label}: <b>PKR ${Number(t.price_pkr).toLocaleString()}</b> / $${t.price_usdt} USDT`).join('<br>')}<br><br>Click <b>Buy Now</b> to add to cart!`;
  } else if (msg.includes('payment') || msg.includes('pay')) {
    reply = `💳 <b>Payment Methods:</b><br>• <b>Binance Pay:</b> ID ${s.binance_id}<br>• <b>EasyPaisa:</b> ${s.easypaisa}<br>• <b>JazzCash:</b> ${s.jazzcash}<br><br>After payment send screenshot on WhatsApp/Telegram ✅`;
  } else if (msg.includes('delivery') || msg.includes('fast') || msg.includes('kab')) {
    reply = `⚡ <b>Delivery:</b> ${s.delivery_time} after payment. We run 24/7!`;
  } else if (msg.includes('safe') || msg.includes('scam') || msg.includes('trust')) {
    reply = `🛡️ <b>100% Safe!</b> ${s.happy_customers} happy customers, ${s.rating} rating. We NEVER ask for your password!`;
  } else if (msg.includes('contact') || msg.includes('support')) {
    reply = `📞 Telegram: ${s.telegram} | WhatsApp: ${s.whatsapp} | Instagram: ${s.instagram}`;
  } else if (msg.includes('refund') || msg.includes('cancel')) {
    reply = `⚠️ All subscriptions are <b>non-refundable</b> once activated. Contact support immediately if any issue!`;
  } else if (msg.includes('hello') || msg.includes('hi') || msg.includes('salaam') || msg.includes('aoa')) {
    reply = `👋 <b>Welcome to Team ESP!</b><br>Ask me about <b>prices</b>, <b>payment</b>, <b>delivery</b>, or <b>support</b>!`;
  } else {
    reply = `🤖 Try asking: <b>prices</b>, <b>payment methods</b>, <b>delivery time</b>, <b>is it safe</b>, or <b>contact</b>`;
  }
...
// Update product info + tiers
app.put('/api/admin/product', auth, (req, res) => {
  const db = readDB();
  db.product = { ...db.product, ...req.body };
  writeDB(db);
...
app.post('/api/admin/product/upload-gallery', auth, upload.array('gallery', 10), (req, res) => {
  ...
  db.product.gallery = [...(db.product.gallery || []), ...urls];
  writeDB(db);
...
app.delete('/api/admin/product/gallery', auth, (req, res) => {
  ...
  writeDB(db);
  res.json({ success: true, gallery: db.product.gallery });
});
...
app.post('/api/admin/faq', auth, (req, res) => {
  const db = readDB();
  db.faq.push({ ...req.body, id: Date.now() });
  writeDB(db);
  res.json({ success: true });
});
